...
const VARIANTS = Object.freeze({
  chrome: {
    id: "chrome-full",
    purpose: "Initial complete Chrome Web Store candidate",
  },
  full: {
    id: "full",
...
function validateManifest(manifest, variant) {
  ...
  for (const host of ["https://civitai.com/*", "https://civitai.red/*"]) {
...
export async function buildRelease({ variantName = "chrome", outputRoot = DEFAULT_DIST_DIR } = {}) {
  ...
  for (const name of PACKAGE_FILES) {
    ...
    const data = normalizeReleaseBytes(name, await readFile(source));
  ...
  const report = {
    ...
    matureSubmission: variantName === "chrome",
...
export async function verifyRelease({ variantName = "chrome", outputRoot = DEFAULT_DIST_DIR } = {}) {
  ...
  for (const name of PACKAGE_FILES) {
    const source = normalizeReleaseBytes(name, await readFile(join(EXTENSION_DIR, name)));
