...
> Store installations update automatically. Version `0.13.1` is the current GitHub release;
...
Chrome handles updates automatically. The `0.13.1` package requests access to both Civitai domains;
...
Firefox updates the add-on automatically. The `0.13.1` package requests access to both Civitai
...
[Releases page](https://github.com/trunksn1/Civitai-MultiHub/releases), currently `v0.13.1`. Every
...
1. Under the `v0.13.1` release, download `civitai-multihub-full-v0.13.1.zip` and its `.sha256` file.
   Make sure the ZIP is an asset on the **Releases** page, not GitHub's automatic source archive.
   All `0.13.1` package variants include both Civitai domains.
...
- Choose comfortable or compact cards and whether videos autoplay.
...
- **General:** density, autoplay, hide viewed, post grouping, viewed-history reset, standalone link
  domain and API key.
