...
test("source UI keeps the corrected hub picker and preview contracts", async () => {
  ...
  assert.match(content, /Finding sources and loading hubs…/);
  assert.match(content, /async function sectionSourceOptions/);
  assert.match(content, /\/api\/v1\/images\?imageId=\$\{imageId\}&limit=1/);
  assert.match(content, /\/api\/v1\/model-versions\/\$\{versionId\}/);
  ...
  assert.match(feedHtml, /id="source-kind-tabs"[^>]+role="tablist"/);
  assert.match(feedHtml, /data-source-kind="user">Creators/);
  assert.match(feedHtml, /data-source-kind="model">Models/);
  assert.match(feedHtml, /data-source-kind="collection">Collections/);
  assert.match(feedHtml, /id="source-input"[^>]+role="combobox"/);
  assert.match(feedHtml, /id="source-suggestions"[^>]+role="listbox"/);
  ...
  assert.match(feedScript, /const SOURCE_SEARCH_DEBOUNCE_MS = 300/);
  assert.match(feedScript, /searchSourceSuggestions\(/);
  assert.match(feedScript, /sourceSearchController\?\.abort\(\)/);
  assert.match(feedScript, /aria-activedescendant/);
  assert.match(feedCss, /\.source-kind-tabs[\s\S]+grid-template-columns:\s*repeat\(3/);
  assert.match(feedCss, /\.source-suggestions[\s\S]+position:\s*absolute/);
  assert.match(feedHtml, /id="hub-picker-trigger"[^>]+aria-haspopup="listbox"/);
  assert.match(feedHtml, /id="hub-picker-options"[^>]+role="listbox"/);
  assert.doesNotMatch(feedHtml, /id="hub-select"/);
  assert.match(feedScript, /function openHubPicker\(/);
  assert.match(feedScript, /hubPickerTypeahead/);
  assert.match(feedScript, /aria-activedescendant/);
  assert.match(feedCss, /\.hub-picker-options[\s\S]+box-shadow/);
  assert.match(feedCss, /\.hub-picker-option\[aria-selected="true"\]/);
  assert.match(feedHtml, /Civitai\.red content profile/);
  assert.match(feedHtml, /civitai\.red only/);
  assert.match(feedHtml, /id="hub-maturity-edit"/);
  assert.match(feedHtml, /id="hub-maturity-clear"[^>]+hidden/);
  assert.match(feedHtml, /id="maturity-editor"[^>]+role="dialog"/);
  assert.match(feedHtml, /Save profile/);
  assert.match(feedScript, /effectiveHubBrowsingLevels\(/);
  assert.match(feedScript, /includeMaturityProfiles: policy === "include"/);
  assert.match(feedScript, /updateAccountBrowsingLevels\(/);
  assert.match(feedScript, /syncSavedMaturityForHub\(/);
  assert.match(feedScript, /host !== "civitai\.red"/);
  assert.match(content, /procedure = "user\.updateContentSettings"/);
  assert.match(content, /location\.hostname !== "civitai\.red"/);
  assert.match(content, /showNsfw: true, domain: "red"/);
  assert.match(background, /"set-browsing-level"/);
  assert.match(feedCss, /\.hub-maturity-card[\s\S]+border-radius/);
  assert.match(feedCss, /\.hub-maturity-level[\s\S]+border-radius:\s*999px/);
